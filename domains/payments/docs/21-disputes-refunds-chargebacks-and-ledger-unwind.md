...
- Compatible with **event sourcing** (“if it wasn’t an event, it never happened”)
...
1. **Ledger-first truth**
   - We never “edit” balances; we append events and recompute.
...
- Balance unwind mechanics for:
  - single buyer charge → multi-seller allocations
...
- We can compute per-seller “net proceeds” and unwind each seller independently.
...
- After shipment: handle via **claims** (damaged, wrong item, counterfeit, not received) rather than “no-reason refunds”.
