...
This document uses the version-system framing to describe how the Catalog defines **Versions**. Per [Catalog terminology](catalog-terminology-and-definitions.md):
...
3. **API validates and normalizes a Version Path**
   ...
   - Normalize to canonical traversal order (identity-safe) per [catalog-version-identity-and-resolution.md](catalog-version-identity-and-resolution.md).
...
The canonical item users search for (e.g., â€œCharizard ex #223â€).
...
- GradedCard (grader â†’ grade scale)
