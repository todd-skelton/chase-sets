...
- `docs/domain/fulfillment-modes-and-pickup-scope.md`
...
If we donâ€™t model this explicitly, we risk:
...
- Any â€œglobal inventoryâ€ is a **projection** (aggregate across locations).

This avoids ambiguity and makes it safe to expand from 1 â†’ many locations.
...
- Listing quantities are reserved/committed against that Locationâ€™s inventory.

Why:

- It prevents â€œglobal poolâ€ confusion and race conditions.
...
- Do not fragment markets into â€œper locationâ€ order books unless absolutely necessary.

Location is a fulfillment concern:

- The selected listingâ€™s Location determines shipping origin and pickup eligibility.
...
- Cart optimization exists in MVP as a secondary UX path (â€œAdd to cartâ€): at checkout, listings may be selected to reduce total cost and/or shipment count.
